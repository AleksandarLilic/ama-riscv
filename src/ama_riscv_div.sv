...
//------------------------------------------------------------------------------
// start conditions
logic op_rem, op_uns, start_a_neg, start_b_neg, start_neg;
...
assign {op_rem, op_uns} = op;
assign start_a_neg = (!op_uns && a[W-1]);
assign start_b_neg = (!op_uns && b[W-1]);
assign start_abs_a = start_a_neg ? (~a + 1'b1) : a;
assign start_abs_b = start_b_neg ? (~b + 1'b1) : b;
assign start_neg = (start_a_neg ^ start_b_neg);
...
assign start_div_by_zero = start_clz_b_empty;
assign start_overflow = (
    !op_uns && (a == 32'h8000_0000) && (b == {W{1'b1}})
);
...
always_comb begin
    special_result = '0;
    if (start_div_by_zero) special_result = op_rem ? a : {W{1'b1}};
    else if (start_overflow) special_result = op_rem ? '0 : a;
    else if (start_less_than_divisor) special_result = op_rem ? a : '0;
...
always_comb begin
    ...
    unique case (state)
        IDLE: begin
            if (start) nx_state = start_special ? SPECIAL : ITER;
        end
